...
class OptimizedVisualization:
    ...
    def create_textured_mesh_overlay_optimized(self, original_image, mesh_vertices, mesh_faces, texture_image, 
        ...
        """OPTIMIZED: Create overlay with vectorized operations - now supports both top and bottom cropping"""
        ...
        # Optimized chest cropping (now supports both top and bottom)
    ...
    def _apply_chest_bottom_cropping_optimized(self, mesh_mask, mesh_vertices, camera_intrinsics, 
        ...
        """OPTIMIZED: Apply chest cropping with vectorized operations - now supports both top and bottom cropping"""
        h, w = image_shape[:2]
        ...
        # Calculate crop lines for both top and bottom
        chest_height = max_y - min_y
        
        # Bottom crop line (crop from bottom)
        bottom_crop_line_y = max_y - int(chest_height * crop_bottom_percentage)
        
        # Top crop line (crop from top)
...
class UltraOptimized3DChestLogoHumanMeshEstimator(HumanMeshEstimator):
    ...
    def process_image_with_textured_3d_chest_logo_optimized(self, img_path, logo_path, output_img_folder, i, 
                                        logo_size=0.15, crop_chest_bottom=True, crop_bottom_percentage=0.5, 
                                        crop_top_percentage=0.1):
        """OPTIMIZED: Process image with parallel processing and caching - now supports both top and bottom cropping"""
        ...
        for batch in dataloader:
            ...
            for person_idx, (vertices_np, cam_trans) in enumerate(person_data):
                ...
                # Optimized overlay creation with both top and bottom cropping
    ...
    def run_textured_3d_chest_logo_pipeline_optimized(self, image_folder, logo_path, out_folder, logo_size=0.15, 
                                   crop_chest_bottom=True, crop_bottom_percentage=0.5, crop_top_percentage=0.1):
        """OPTIMIZED: Run pipeline with parallel image processing - now supports both top and bottom cropping"""
        ...
        print(f"Bottom crop percentage: {crop_bottom_percentage}")
        print(f"Top crop percentage: {crop_top_percentage}")
...
# Updated usage example with both top and bottom cropping
