...
class Enhanced3DVisualization:
    ...
    def create_textured_mesh_overlay(self, original_image, mesh_vertices, mesh_faces, texture_image, 
        ...
        """Create overlay of textured 3D mesh on original image with optional chest bottom cropping"""
        # Render the textured mesh
        rendered_mesh = self.render_textured_mesh_to_image(
            mesh_vertices, mesh_faces, texture_image, uv_coords,
            camera_intrinsics, camera_translation, original_image.shape
        ...
        # NEW: Apply chest bottom cropping if requested
        if crop_chest_bottom:
            mesh_mask = self._apply_chest_bottom_cropping(
    ...
    def _apply_chest_bottom_cropping(self, mesh_mask, mesh_vertices, camera_intrinsics, 
                               camera_translation, image_shape, crop_percentage=0.5):
        """Apply 10% cropping from bottom of chest region in the mesh mask"""
        h, w = image_shape[:2]
        ...
        # Find chest region in 2D (where mesh is visible)
        visible_pixels = np.where(mesh_mask)
        
        if len(visible_pixels[0]) == 0:
            return mesh_mask
        
        # Get bounding box of visible mesh region
        min_y = np.min(visible_pixels[0])
        max_y = np.max(visible_pixels[0])
        min_x = np.min(visible_pixels[1])
        max_x = np.max(visible_pixels[1])
        
        # Calculate chest region height and crop line
        chest_height = max_y - min_y
        crop_line_y = max_y - int(chest_height * crop_percentage)
        
        # Create modified mask by removing bottom 10% of chest region
        modified_mask = mesh_mask.copy()
        
        # Remove pixels below the crop line within the chest region
        for y in range(crop_line_y, max_y + 1):
            for x in range(min_x, max_x + 1):
                if y < h and x < w:
                    modified_mask[y, x] = False
...
class UltraEnhanced3DChestLogoHumanMeshEstimator(HumanMeshEstimator):
    ...
    def process_image_with_textured_3d_chest_logo(self, img_path, logo_path, output_img_folder, i, 
        ...
        """Process image with textured 3D chest logo and proper overlay with optional chest cropping"""
        img_cv2 = cv2.imread(str(img_path))
        ...
        for batch in dataloader:
            ...
            for person_idx in range(output_vertices.shape[0]):
                ...
                # Create individual overlay for this person WITH chest cropping
                overlay_image, rendered_mesh = self.visualizer.create_textured_mesh_overlay(
                    img_cv2, vertices_np, self.smpl_model.faces, texture_image, uv_coords,
                    cam_int, cam_trans, blend_alpha=0.8, 
                ...
                # Update combined overlay by blending this person's mesh onto it WITH chest cropping
                combined_overlay, _ = self.visualizer.create_textured_mesh_overlay(
                    combined_overlay, vertices_np, self.smpl_model.faces, texture_image, uv_coords,
                    cam_int, cam_trans, blend_alpha=0.8,
                    crop_chest_bottom=crop_chest_bottom, crop_percentage=crop_percentage
                ...
                print(f"Completed processing person {person_count} with textured 3D chest logo overlay (cropped bottom {crop_percentage*100}%)")
    ...
    def run_textured_3d_chest_logo_pipeline(self, image_folder, logo_path, out_folder, logo_size=0.15, 
                                       crop_chest_bottom=True, crop_percentage=0.5):
        """Run textured 3D chest logo pipeline with proper overlay and optional chest cropping"""
        if not os.path.exists(out_folder):
            os.makedirs(out_folder)
        print("Directory created suc")
        from glob import glob
        ...
        print(f"Using logo: {logo_path}")
        print(f"Logo size: {logo_size}")
        print(f"Chest bottom cropping: {crop_chest_bottom}")
        if crop_chest_bottom:
            print(f"Crop percentage: {crop_percentage*100}%")
        print(f"Output folder: {out_folder}")
        print(f"Found {len(images_list)} images to process")
