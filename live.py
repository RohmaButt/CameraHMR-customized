...
class DynamicChestLogo3D:
    ...
    def get_chest_vertices_from_joints(self, vertices, joints_3d):
        ...
        relative_pos = vertices - chest_center[None, :]
        ...
        # Vectorized mask for chest region
        mask = (
            (np.abs(u_coord) <= chest_width / 2) &
            (np.abs(v_coord) <= chest_height / 2) &
            (depth_coord >= -0.05) &
            (depth_coord <= 0.1)  # Front-facing vertices
        )
        
        chest_vertices_indices = np.where(mask)[0]
        
        return chest_vertices_indices
    ...
    def validate_chest_vertices_dynamic(self, vertices, joints_3d):
        ...
        try:
            ...
            relative_chest = chest_vertices - chest_center[None, :]
            depths = np.dot(relative_chest, chest_normal)
    ...
    def _create_dynamic_uv_mapping(self, vertices, faces, chest_center, right_vector, up_vector, chest_normal, joints_3d):
        ...
        if not chest_vertices_indices.size:
            return self._create_fallback_uv_mapping(vertices, chest_center, right_vector, up_vector)
        
        # Calculate distances to chest center
        distances_to_chest = np.linalg.norm(vertices - chest_center[None, :], axis=1)
        chest_vertices = vertices[chest_vertices_indices]
        max_chest_distance = np.max(np.linalg.norm(chest_vertices - chest_center[None, :], axis=1))
        
        # Weight calculation
        chest_weights = np.exp(-distances_to_chest / (max_chest_distance * 0.5))
        
        # Transform to chest coordinate system
        centered_vertices = vertices - chest_center[None, :]
        ...
        chest_centered = chest_vertices - chest_center[None, :]
...
class PostProcessingTransparency:
    ...
    def create_magenta_mask(self, image):
        ...
        color_diff = np.linalg.norm(image_rgb - self.magenta_color[None, None, :], axis=2)
...
class OcclusionAwareVisualization:
    ...
    def render_multiple_meshes_with_occlusion(self, mesh_data_list, camera_intrinsics, image_shape):
        ...
        for person_idx, (vertices, faces, texture, uv_coords, cam_trans, person_id) in enumerate(mesh_data_list):
            ...
            vertices_cam = vertices + cam_trans[None, :]
            ...
            if len(valid_indices) > 0:
                valid_vertices_cam = vertices_cam[valid_indices]
                valid_depths = depths[valid_indices]
                vertices_2d[valid_indices, 0] = (valid_vertices_cam[:, 0] * fx / valid_depths) + cx
                vertices_2d[valid_indices, 1] = (valid_vertices_cam[:, 1] * fy / valid_depths) + cy
            
            # Process faces and create triangle data
            for face in faces:
                if np.all(valid_depth_mask[face]):
    ...
    def _rasterize_triangle_with_occlusion(self, image, z_buffer, person_id_buffer, triangle_data):
        ...
        # Calculate barycentric coordinates vectorized
        w0 = ((v1[1] - v2[1]) * (xx - v2[0]) + (v2[0] - v1[0]) * (yy - v2[1])) / denom
        w1 = ((v2[1] - v0[1]) * (xx - v2[0]) + (v0[0] - v2[0]) * (yy - v2[1])) / denom
        w2 = 1 - w0 - w1
        ...
        # Filter to inside pixels
        w0_inside = w0[inside_mask]
        w1_inside = w1[inside_mask]
        w2_inside = w2[inside_mask]
        xx_inside = xx[inside_mask]
        yy_inside = yy[inside_mask]
        
        # Interpolate depth
        depth_inside = w0_inside * d0 + w1_inside * d1 + w2_inside * d2
        
        # Depth test
        current_z = z_buffer[yy_inside, xx_inside]
        depth_bias = 1e-6
        update_mask = depth_inside < current_z - depth_bias
        
        if not np.any(update_mask):
            return
        
        # Update buffers for pixels that pass depth test
        update_indices = np.where(update_mask)[0]
        yy_update = yy_inside[update_indices]
        xx_update = xx_inside[update_indices]
        depth_update = depth_inside[update_indices]
        
        z_buffer[yy_update, xx_update] = depth_update
        person_id_buffer[yy_update, xx_update] = person_id
        
        # Interpolate UV for update pixels
        w0_update = w0_inside[update_indices]
        w1_update = w1_inside[update_indices]
        w2_update = w2_inside[update_indices]
        
        u_update = w0_update * uv0[0] + w1_update * uv1[0] + w2_update * uv2[0]
        v_update = w0_update * uv0[1] + w1_update * uv1[1] + w2_update * uv2[1]
        
        # Sample texture
        tex_x_update = np.clip(u_update * texture_w, 0, texture_w - 1).astype(int)
        tex_y_update = np.clip(v_update * texture_h, 0, texture_h - 1).astype(int)
        
        sampled_colors = texture[tex_y_update, tex_x_update]
        
        # Apply to image
        if texture.shape[2] == 4:
            alpha = sampled_colors[:, 3] / 255.0
            significant_alpha = alpha > 0.1
            if np.any(significant_alpha):
                update_sig_indices = np.where(significant_alpha)[0]
                yy_sig = yy_update[update_sig_indices]
                xx_sig = xx_update[update_sig_indices]
                alpha_sig = alpha[update_sig_indices]
                sampled_rgb_sig = sampled_colors[update_sig_indices, :3].astype(np.float32)
                current_img_sig = image[yy_sig, xx_sig].astype(np.float32)
                blended_sig = alpha_sig[:, None] * sampled_rgb_sig + (1 - alpha_sig[:, None]) * current_img_sig
                image[yy_sig, xx_sig] = blended_sig.astype(np.uint8)
        else:
            image[yy_update, xx_update] = sampled_colors[:, :3]
...
# NEW: Live Camera Thread Handler
class LiveCameraHandler:
    """Handle live camera feed processing with threading for real-time performance"""
    
    def __init__(self, estimator, logo_path, logo_size=0.15, fps_target=30):
        ...
        # Load logo once
        self.logo_image = self.estimator.load_and_validate_logo_cached(logo_path)
    ...
    def start_processing(self):
        ...
        print("Live processing thread started")
    ...
    def stop_processing(self):
        ...
        print("Live processing thread stopped")
    ...
    def _processing_loop(self):
        ...
        while self.is_running:
            try:
                ...
                # Put result in queue (non-blocking, drop old results if queue full)
                try:
                    self.result_queue.put((result, timestamp, processing_time), block=False)
                except queue.Full:
    ...
    def _process_single_frame(self, frame):
        """Process a single frame with logo overlay"""
        try:
            # Human detection
            det_out = self.estimator.detector(frame)
            det_instances = det_out['instances']
            valid_idx = (det_instances.pred_classes == 0) & (det_instances.scores > 0.5)
            
            if not valid_idx.any():
                return frame  # No humans detected, return original
            ...
            cam_int = self.estimator.get_cam_intrinsics(frame)
            dataset = Dataset(frame, bbox_center, bbox_scale, cam_int, False, "live_camera")
            
            dataloader = torch.utils.data.DataLoader(dataset, batch_size=8, shuffle=False, num_workers=2, pin_memory=True)
            ...
            for batch in dataloader:
                batch = recursive_to(batch, self.estimator.device)
                ...
                for person_idx in range(output_vertices.shape[0]):
                    ...
                    # Create texture with dynamic chest calculation
                    texture_image, uv_coords, logo_bounds = self.estimator.chest_logo_3d.create_chest_focused_texture_dynamic(
                        vertices_np, self.estimator.smpl_model.faces, joints_3d, self.logo_image, self.logo_size
                    )
                    
                    # Store mesh data
                    mesh_data = (
                        vertices_np,
                        self.estimator.smpl_model.faces,
                        texture_image,
                        uv_coords,
                        cam_trans,
                        person_count
            ...
            if not all_mesh_data:
                return frame  # No valid mesh data
            
            # Create occlusion-aware overlay
            overlay_image, rendered_mesh, debug_info = self.estimator.visualizer.create_occlusion_aware_overlay(
                frame, all_mesh_data, cam_int, blend_alpha=0.8,
                crop_chest_bottom=True,
                crop_bottom_percentage=0.5,
                crop_top_percentage=0.1
            )
            
            # Post-processing to remove magenta and create clean result
            transparent_overlay, clean_overlay = self.estimator.visualizer.post_processor.remove_magenta_create_transparent(
                overlay_image, rendered_mesh, frame
            )
    ...
    def get_fps_info(self):
        """Get current FPS and processing time info"""
        if len(self.fps_counter) > 1:
            avg_fps = len(self.fps_counter) / (self.fps_counter[-1] - self.fps_counter[0])
        else:
            avg_fps = 0
...
# Updated main estimator class with live camera support
class DynamicChestLogoHumanMeshEstimator(HumanMeshEstimator):
    def __init__(self, *args, **kwargs):
        ...
        self.visualizer = OcclusionAwareVisualization()
    ...
    def run_live_camera(self, logo_path, camera_id=0, logo_size=0.15, 
                       display_width=1280, display_height=720, save_video=False, output_video_path=None):
        """
        Run live camera processing with real-time logo overlay
        
        Args:
            logo_path: Path to logo image
            camera_id: Camera device ID (0 for default camera)
            logo_size: Size of logo on mesh
        ...
        """
        print("=== LIVE CAMERA MODE WITH DYNAMIC CHEST LOGO ===")
        ...
        if save_video and output_video_path:
            ...
            video_writer = cv2.VideoWriter(output_video_path, fourcc, 20.0, (display_width, display_height))
            print(f"Recording to: {output_video_path}")
        
        # Initialize live camera handler
        self.live_camera_handler = LiveCameraHandler(self, logo_path, logo_size, fps_target=30)
        self.live_camera_handler.start_processing()
        ...
        try:
            while True:
                ...
                # Add frame to processing queue
                self.live_camera_handler.add_frame(frame)
                
                # Get processed result
                result_data = self.live_camera_handler.get_result()
                if result_data is not None:
                    processed_frame, timestamp, processing_time = result_data
                ...
                # Update FPS counter
                self.live_camera_handler.update_fps_counter()
                
                # Add performance info to display
                current_time = time.time()
                if current_time - last_fps_update >= 1.0:  # Update every second
                    avg_fps, avg_processing_time = self.live_camera_handler.get_fps_info()
                    last_fps_update = current_time
                else:
                    avg_fps, avg_processing_time = self.live_camera_handler.get_fps_info()
                
                # Add text overlay with performance info
                display_frame_with_info = display_frame.copy()
                cv2.putText(display_frame_with_info, f"FPS: {avg_fps:.1f}", (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(display_frame_with_info, f"Processing: {avg_processing_time*1000:.1f}ms", (10, 60), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(display_frame_with_info, "Press 'q' to quit, 's' to save frame", (10, display_height - 20), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                
                # Display frame
                cv2.imshow('Live Camera with Dynamic Chest Logo', display_frame_with_info)
                ...
                if key == ord('q'):
                    ...
                    save_path = f"live_camera_frame_{timestamp_str}.jpg"
            ...
            if self.live_camera_handler:
                self.live_camera_handler.stop_processing()
    ...
    def process_image_with_dynamic_chest_logo(self, img_path, logo_path, output_img_folder, i, 
        ...
        if debug_info['depth_map'] is not None:
            ...
            finite_mask = np.isfinite(debug_info['depth_map'])
            if np.any(finite_mask):
                depth_min = np.min(debug_info['depth_map'][finite_mask])
                depth_max = np.max(debug_info['depth_map'][finite_mask])
                depth_normalized = ((debug_info['depth_map'] - depth_min) / 
                                  (depth_max - depth_min + 1e-8) * 255).astype(np.uint8)
...
if __name__ == "__main__":
    ...
    def make_parser():
        parser = argparse.ArgumentParser(description='Enhanced Dynamic CameraHMR with Live Camera Support')
        
        # Mode selection
        parser.add_argument("--mode", type=str, choices=['image', 'live'], default='image',
                           help="Processing mode: 'image' for batch processing, 'live' for camera")
        ...
        parser.add_argument("--image_folder", type=str, help="Path to input image folder (for image mode).")
        parser.add_argument("--output_folder", type=str, help="Path to output folder (for image mode).")
        ...
        # Live camera mode parameters
        parser.add_argument("--camera_id", type=int, default=0, help="Camera device ID (for live mode)")
        parser.add_argument("--display_width", type=int, default=1280, help="Display window width (for live mode)")
        parser.add_argument("--display_height", type=int, default=720, help="Display window height (for live mode)")
        parser.add_argument("--save_video", action='store_true', help="Save output video (for live mode)")
        parser.add_argument("--output_video_path", type=str, help="Output video file path (for live mode)")
    ...
    if args.mode == 'image':
        ...
        if not args.image_folder or not args.output_folder:
            print("Error: --image_folder and --output_folder are required for image mode")
            exit(1)
